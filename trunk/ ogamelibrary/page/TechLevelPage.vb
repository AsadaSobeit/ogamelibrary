...
#Region "Regular Expression"
        ...
        '<tr>(?:<td class=l><a href=infos[.]php[?]session=\w{12}&gid=\d+(?:&PHPSESSID=\w{32})?>.*?</a></td>)?<td class=l><a href=infos[.]php[?]session=(?<session>\w{12})&gid=(?<gid>\d+)(?:&PHPSESSID=\w{32})?>.*?</a>(?:</a> [(]\D+(?<level>\d+) ?[)])?<br>.*?(?:</td>)?<td class=[k]>(?:<script src="lang/en/cnt[.]js" type="text/javascript"> </script><div id="bxx" class="z"></div><SCRIPT language=JavaScript>\s+pp="(?<seconds>\d+)";\s+pk="\k<gid>";\s+pl="(?<planetid>\d+)";\s+ps="\k<session>";\s+t[(][)];\s+</script>\s+|</td>)</td></tr>
